...
def random_pair_generator(m,r,epsilon):
  ...
  return np.concatenate((u.copy(),np.zeros(2**m-binom))), y.copy()

def random_goodpair_generator(m,r=1,epsilon=0):
  ...
  y=(x+np.random.binomial(1,epsilon,2**m))%2
  u1=FHT_decode(y)
  return np.concatenate((u1.copy(),np.zeros(2**m-binom))),y.copy()
...
def complementary(m,r):
  '''Returns a conjugate matrix H such that for every v in RM(m,r), Hv=0 and H is full-rank.
     m,r are the integers representing the parameters of RM(m,r) code.
...
def max_likelihood(m,r,y):
  '''Maximum likelihood decoder, decodes y as a noisy RM(m,r) codeword.
     m,r are integers representing the parameters of RM(m,r) code, y is the length 2^m array representing the noisy codeword.
  ...
  argmin=np.zeros(2**m)
  min=np.sum(y)
  for i in range(2**binom):
    x=RM_encoder(m,r,num_to_binom(i,binom))
    if min >= np.sum(abs(x-y)):
      min=np.sum(abs(x-y))
      argmin=x
  return argmin

def accuracy(x, y):
  '''Compares x and y. Returns 0 if exists a bit i s.t. x[i]=y[i] and returns 1 otherwise.
